...
def mtx_to_mm (matrixname):
   ...
   matrixName='matrices/'+matrixname+'.mtx'
   ...
   mtx_mat = [ln.strip() for ln in mtx_mat]
   ...
   if len(contents[0]) == 2:
      contents = [[p[0][0], p[0][1], float(p[1])] for p in zip(contents, list(range(1, len(contents)+1)))]               
   # Convert indices to ints and sort
   contents = map(lambda elt: [int(elt[0]), int(elt[1]), elt[2]],
                  contents)
   contents = [[int(elt[0]), int(elt[1]), elt[2]] for elt in contents]               
   # contents = sorted(contents, lambda x, y: -1 if lexorder(x,y) else 1)
   contents = sorted(contents, key=functools.cmp_to_key(lambda x, y: -1 if lexorder(x,y) else 1) )
   # This test for zeros is kind of bogus, but seems to work:
   contents = [x for x in contents if float(x[2])!=0.0]
